...
const AssetInventory = () => {
  ...
  const slicedData =
    assets && Array.isArray(assets) ? assets.slice(startIndex, endIndex) : [];
  ...
  // console.log(originalData, 'orginal data');
  const getDepartment = () => {
    const config: any = {};
    try {
  ...
  console.log(SiteFilter, categoryFilter, departmentFilter);
  ...
  useEffect(() => {
    applyFilters();
  }, [categoryFilter, locationFilter, departmentFilter, SiteFilter]);
    ...
    setAssets((prevAssets: any) => {
      ...
      if (categoryFilter !== '') {
        filteredData = filteredData.filter(
          (res) => res.category_name === categoryFilter
        );
      }

      if (locationFilter !== '') {
        filteredData = filteredData.filter(
          (res) => res.location_name === locationFilter
        );
      }
      if (SiteFilter !== '') {
        filteredData = filteredData.filter(
          (res) => res.company_name === SiteFilter
        );
      }
      if (departmentFilter !== '') {
        filteredData = filteredData.filter(
          (res) => res.employee_name === departmentFilter
        );
      }
  ...
  const handlePageChange = (direction: string) => {
    setPage((prevPage) => {
      if (direction === 'next' && (assets?.length ?? 0) > showEntries) {
        return prevPage + 1;
          ...
          <div className="d-flex w-100">
            ...
            <div className="d-flex justify-content-end align-items-center w-75">
              <Form.Group className="py-1 mx-2" controlId="location">
                <InputGroup className="py-1  float-end">
                  <InputGroup.Text
                    id="basic-addon1"
                    className="bg-transparent px-2"
                  >
                    <HiFilter className="me-1 theme-danger" />
                  </InputGroup.Text>
                  <Form.Select
                    ...
                    onChange={(e: any) => filterSites(e.target.value)}
                  >
                    <option onClick={() => setAssets(originalData)} value="">
                      Select Sites
                    </option>
                    {CompanyData?.map((item: any) => (
                      <option
                        value={item?.company_name}
                        className="theme-font fs-7"
                      >
                        {item?.company_name}
                      </option>
                    ))}
              ...
              <Form.Group className="py-1 mx-2" controlId="department">
                <InputGroup className="py-1">
                  ...
                  >
                    ...
                    {department
                      ?.filter((item: any) => item.company_id == company)
                      .map((item: any) => (
              ...
              {Array?.isArray(slicedData) && assets ? (
                slicedData?.map((asset: any) => (
                  <tr>
                    ...
                    <td>
                      {asset?.employee_id ? (
                        <button className="bg-success fs-7 text-white border-0 rounded px-2">
                          Assigned
                        </button>
                      ) : (
                        <button className="bg-warning fs-7 text-white border-0 rounded px-2">
                          Un&nbsp;assigned
                        </button>
                      )}
                    </td>
                    <td>
                      <div className="d-flex justify-content-end align-items-center">
                        ...
                        >
                          <Dropdown.Item
                            onClick={() =>
                              handleArchive(JSON.parse(asset?.asset_unique_id))
                            }
                            className="theme-font fs-7"
                            as="button"
                          >
                          ...
                          <Dropdown.Item
                            onClick={() =>
                              handleDelete(JSON.parse(asset?.asset_unique_id))
                            }
                            className="theme-font fs-7"
                            as="button"
                          >
            ...
            <div className="w-100 d-flex justify-content-end">
              <div className="pagination">
                ...
                <button
                  ...
                  disabled={
                    assets?.length <= showEntries ||
                    assets?.length <= page * showEntries
                  }
                >
                  Next
                </button>
